...
char *Get_Next_MaplistTxt_Map ()
{
	...
	for (i = 0; i<20 && removed != mapcount-1 && last_maps_played[i]; i++) //for last_maps_played
	{
		for (j=0; j < mapcount; j++)
		{
			...
			{	
				maplisttxt[j]= "";
				//gi.dprintf(DEVELOPER_MSG_GAME, "EEEEEEEE Removing %s\n",campaign_spots[checknum].bspname);
				removed++;
				break;
			}
		}
	...
	if (*sv_maplist->string) 
	{
		...
		else
		{
			...
			while (t != NULL)
			{
				...
				{	
					//gi.dprintf(DEVELOPER_MSG_GAME, "rwef %s %s\n",mapname, t);
				...
				if (Q_stricmp(t, mapname) == 0)   //if the running map is on maplist
				{
					...
					else
					{
						...
						else
						{
							...
							check = t;
							strcat (check,"1");
							//gi.dprintf(DEVELOPER_MSG_GAME, "%s kljfdsjlk\n",check);
							if (MapExists(check))
							{
								safe_bprintf (PRINT_HIGH, "Next map: %s \n", check);
								BeginIntermission (CreateTargetChangeLevel (check));
...
//faf end
	...
	for(i=0; i < MAX_TEAMS;i++) 
	{
		...
		/*	gi.dprintf(DEVELOPER_MSG_GAME, "time_to_win [%i] = %f\n", i, team_list[i]->time_to_win);
			gi.dprintf(DEVELOPER_MSG_GAME, "delay       [%i] = %f\n", i, delay);
			gi.dprintf(DEVELOPER_MSG_GAME, "level.time  [%i] = %f\n", i, level.time); */
