...
static void CDAudio_Eject(void)
{
	...
	if (dwReturn)
		Com_DPrintf(DEVELOPER_MSG_CD, "MCI_SET_DOOR_OPEN failed (%u)\n", (unsigned int)dwReturn);
}
...
static void CDAudio_CloseDoor(void)
{
	...
	if (dwReturn)
		Com_DPrintf(DEVELOPER_MSG_CD, "MCI_SET_DOOR_CLOSED failed (%u)\n", (unsigned int)dwReturn);
}
...
static int CDAudio_GetAudioDiskInfo(void)
{
	...
	if (dwReturn)
	{
		Com_DPrintf(DEVELOPER_MSG_CD, "CDAudio: drive ready test - get status failed\n");
		return -1;
	}
	if (!mciStatusParms.dwReturn)
	{
		Com_DPrintf(DEVELOPER_MSG_CD, "CDAudio: drive not ready\n");
		return -1;
	}
	...
	if (dwReturn)
	{
		Com_DPrintf(DEVELOPER_MSG_CD, "CDAudio: get tracks - status failed\n");
		return -1;
	}
...
void CDAudio_Play2(int track, qboolean looping)
{
	...
	if (dwReturn)
	{
		Com_DPrintf(DEVELOPER_MSG_CD, "MCI_STATUS failed (%u)\n", (unsigned int)dwReturn);
		return;
	}
	...
	if (dwReturn)
	{
		Com_DPrintf(DEVELOPER_MSG_CD, "CDAudio: MCI_PLAY failed (%u)\n", (unsigned int)dwReturn);
		return;
	}
...
void CDAudio_Stop(void)
{
	...
	if (dwReturn)
		Com_DPrintf(DEVELOPER_MSG_CD, "MCI_STOP failed (%u)", (unsigned int)dwReturn);
...
void CDAudio_Pause(void)
{
	...
	if (dwReturn)
		Com_DPrintf(DEVELOPER_MSG_CD, "MCI_PAUSE failed (%u)", (unsigned int)dwReturn);
...
/*	dwReturn = mciSendCommand(wDeviceID, MCI_RESUME, MCI_WAIT, NULL); */
	...
	if (dwReturn)
	{
		Com_DPrintf(DEVELOPER_MSG_CD, "CDAudio: MCI_STATUS failed (%u)\n", (unsigned int)dwReturn);
		return;
	}
...
LRESULT CDAudio_MessageHandler(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (wParam)
	{
		...
		case MCI_NOTIFY_FAILURE:
			Com_DPrintf(DEVELOPER_MSG_CD, "MCI_NOTIFY_FAILURE\n");
			CDAudio_Stop ();
			cdValid = false;
			break;

		default:
			Com_DPrintf(DEVELOPER_MSG_CD, "Unexpected MM_MCINOTIFY type (%Iu)\n", wParam);
			return 1;
	}
...
int CDAudio_Init(void)
{
	...
	if (dwReturn)
	{
		Com_Printf("CDAudio_Init: MCI_OPEN failed (%u)\n", (unsigned int)dwReturn);
		return -1;
	}
	...
	if (dwReturn)
	{
		Com_Printf("MCI_SET_TIME_FORMAT failed (%u)\n", (unsigned int)dwReturn);
...
void CDAudio_Shutdown(void)
{
	...
	if (mciSendCommand(wDeviceID, MCI_CLOSE, MCI_WAIT, (DWORD_PTR)NULL))
		Com_DPrintf(DEVELOPER_MSG_CD, "CDAudio_Shutdown: MCI_CLOSE failed\n");
}
