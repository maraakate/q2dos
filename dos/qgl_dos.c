...
{
	fprintf( log_fp, "%s( 0x%x )\n", "glBegin", mode );
	dllBegin( mode );
}

static void APIENTRY logBindTexture(GLenum target, GLuint texture)
{
	fprintf( log_fp, "%s( 0x%x, %u )\n", "glBindTexture", target, texture );
...
{
	fprintf( log_fp, "%s( 0x%x, 0x%x, 0x%x )\n", "glTexEnvi", target, pname, param );
...
#ifdef GL_DLSYM /* dynamic loading : */
#define GPA(func) QGL_dlsym(#func)
static void * QGL_dlsym (const char *sym) {
	static char name[128] = { '_', 0 };
	strcpy(&name[1],sym);
	return dlsym(opengl_dxe, name);
}
#else /* static build */
#define GPA(func) func
#endif
...
qboolean QGL_Init(const char *dllname)
{
	...
	if (rc < 0) rc = DMESA_LoadAPI (opengl_dxe);
	if (rc < 0) rc = SAGE_LoadAPI (opengl_dxe);
	if (rc < 0) rc = FXMESA_LoadAPI (opengl_dxe);
	if (rc < 0) {
		ri.Con_Printf (PRINT_ALL, "Unable to find a supported API for DOSGL.\n");
		return false;
	}
	ri.Con_Printf(PRINT_ALL, "DOSGL: driver using %s API.\n", DOSGL_APIName());
