...
static unsigned int snd_emu20kx_buffer_init(struct emu20kx_card_s *card,struct mpxplay_audioout_info_s *aui)
{
 ...
 card->pcmout_bufsize=MDma_get_max_pcmoutbufsize(EMU20KX_PAGESIZE,bytes_per_sample);
 card->dm=pds_dpmi_dos_allocmem( EMU20KX_MAXPAGES*sizeof(uint32_t)// virtualpage
...
{
 unsigned int i,gctl,trnctl;
 //unsigned int ctl_amoplo;
...
static int EMU20KX_adetect(struct mpxplay_audioout_info_s *aui)
{
 ...
 if(pcibios_search_devices(&emu20kx_devices[0],card->pci_dev)!=PCI_SUCCESSFUL)
  goto err_adetect;
