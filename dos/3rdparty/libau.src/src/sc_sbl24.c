...
static unsigned int snd_live24_buffer_init(struct emu10k1_card *card,struct mpxplay_audioout_info_s *aui)
{
 ...
 card->pcmout_bufsize=MDma_get_max_pcmoutbufsize(CA0106_DMABUF_ALIGN,bytes_per_sample);
...
_farnspokel((unsigned int)&table_base[i*2+1],period_size_bytes<<16);
 ...
 snd_ca0106_ptr_write(card, PLAYBACK_LIST_ADDR, channel, (uint32_t)card->virtualpagetable);
