...
static unsigned int snd_emu10kx_buffer_init(struct emu10k1_card *card,struct mpxplay_audioout_info_s *aui)
{
 ...
 card->pcmout_bufsize=MDma_get_max_pcmoutbufsize(EMUPAGESIZE,2);
 card->dm=pds_dpmi_dos_allocmem( MAXPAGES*sizeof(uint32_t)       // virtualpage
...
_farnspokel((unsigned int)&card->virtualpagetable[pagecount], pcmbufp | pagecount);
  pcmbufp+=EMUPAGESIZE*2;
 }
 for( ; pagecount<MAXPAGES; pagecount++)
_farnspokel((unsigned int)&card->virtualpagetable[pagecount],((uint32_t)card->silentpage)<<1);
#else
 for(pagecount = 0; pagecount < (card->pcmout_bufsize/EMUPAGESIZE); pagecount++){
...
_farnspokel((unsigned int)&table_base[i*2],(uint32_t)((char *)card->pcmout_buffer+(i*period_size_bytes)));
_farnspokel((unsigned int)&table_base[i*2+1],period_size_bytes<<16);
 }
#else
 for(i=0; i<AUDIGY2_P16V_PERIODS; i++) {
 ...
 emu10k1_ptr20_write(card, PLAYBACK_LIST_ADDR, channel, (uint32_t)card->virtualpagetable);
...
static void SBLIVE_card_info(struct mpxplay_audioout_info_s *aui)
{
 ...
 sprintf(aui->infostr,"SB %s on port:%4.4lX irq:%u (bits:16%s)",
         ((card->card_capabilities->longname)? card->card_capabilities->longname:card->pci_dev->device_name),
         card->iobase, card->irq, ((card->chips&EMU_CHIPS_24BIT)? ",24":"") );
}
