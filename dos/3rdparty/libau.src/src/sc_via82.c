...
{
 via82xx_AC97Codec_ready(card->iobase);
 outl(card->iobase + VIA_REG_OFFSET_TABLE_PTR,(unsigned long)card->virtualpagetable);
 pds_delay_10us(2);
 via82xx_AC97Codec_ready(card->iobase);
}
...
static int VIA82XX_adetect(struct mpxplay_audioout_info_s *aui)
{
 ...
 // alloc buffers
 card->pcmout_bufsize=MDma_get_max_pcmoutbufsize(PCMBUFFERPAGESIZE,2);
...
_farnspokel((unsigned int)&card->virtualpagetable[pagecount*2],pcmbufp);
  if(pagecount<(card->pcmout_pages-1))
_farnspokel((unsigned int)&card->virtualpagetable[pagecount*2+1],PCMBUFFERPAGESIZE); // 0x00001000; // period continues to the next
  else
_farnspokel((unsigned int)&card->virtualpagetable[pagecount*2+1],VIA_TBL_BIT_EOL|PCMBUFFERPAGESIZE); // 0x80001000; // buffer boundary
  pcmbufp+=PCMBUFFERPAGESIZE;
 }
#else
 for(pagecount=0;pagecount<card->pcmout_pages;pagecount++){
  card->virtualpagetable[pagecount*2]=pcmbufp;
  if(pagecount<(card->pcmout_pages-1))
