...
_farnspokel((unsigned int)&table_base[i*2],(uint32_t)((char *)card->pcmout_buffer+(i*card->period_size_bytes)));
_farnspokel((unsigned int)&table_base[i*2+1],period_size_samples);
}
#else
 for(i=0; i<ICH_DMABUF_PERIODS; i++){
...
#endif
 snd_intel_write_32(card,ICH_PO_BDBAR_REG,(uint32_t)card->virtualpagetable);
...
static void INTELICH_card_info(struct mpxplay_audioout_info_s *aui)
{
 ...
 sprintf(aui->infostr,"ICH: %s on port:%4.4lX irq:%u (type:%s, bits:16%s)",
